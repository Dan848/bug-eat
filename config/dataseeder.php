<?php

return [
    'users' => [
        [
            'name' => 'Clelia',
            'surname' => 'Fradella',
            'email' => 'cleliafradella@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Adriano',
            'surname' => 'Grimaldi',
            'email' => 'adrianogrimaldi@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Marco',
            'surname' => 'Acciarri',
            'email' => 'marcoacciarri@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Bryan',
            'surname' => 'Lucchetta',
            'email' => 'bryanlucchetta@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Samuel',
            'surname' => 'Panicucci',
            'email' => 'samuelpanicucci@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Gabriele',
            'surname' => 'Magni',
            'email' => 'gabrielemagni@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Giuliano',
            'surname' => 'Gostifini',
            'email' => 'giulianogostifini@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Giuseppe',
            'surname' => 'Funicello',
            'email' => 'giuseppefunicello@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Valeria',
            'surname' => 'Marta',
            'email' => 'valeriamarta@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Marta',
            'surname' => 'Seminara',
            'email' => 'martaseminara@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Margherita',
            'surname' => 'Ghidini',
            'email' => 'margheritaghidini@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Marilena',
            'surname' => 'Costa',
            'email' => 'marilenacosta@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Filippo',
            'surname' => 'Mangione',
            'email' => 'filippomangione@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Liam',
            'surname' => 'Wilson',
            'email' => 'liamwilson@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Mia',
            'surname' => 'Thompson',
            'email' => 'miathompson@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Benjamin',
            'surname' => 'White',
            'email' => 'benjaminwhite@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Charlotte',
            'surname' => 'Harris',
            'email' => 'charlotteharris@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'William',
            'surname' => 'Martin',
            'email' => 'williammartin@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Ava',
            'surname' => 'Garcia',
            'email' => 'avagarcia@mail.com',
            'password' => 'bugmaker'
        ],
        // Boolean Users
        [
            'name' => 'Chiara',
            'surname' => 'Sucato',
            'email' => 'chiarasucato@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Marika',
            'surname' => 'Di Blasio',
            'email' => 'marikadiblasio@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Gaetano',
            'surname' => 'Spampinato',
            'email' => 'gaetanospampinato@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Andrea',
            'surname' => 'Collura',
            'email' => 'andreacollura@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Marco',
            'surname' => 'Bione',
            'email' => 'marcobione@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Aniello',
            'surname' => 'Rossi',
            'email' => 'aniellorossi@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Giuseppe',
            'surname' => 'Lucà',
            'email' => 'giuseppeluca@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Vito',
            'surname' => 'De Palma',
            'email' => 'vitodepalma@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Michele',
            'surname' => 'Libralato',
            'email' => 'michelelibralato@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Daniele',
            'surname' => 'Ceravolo',
            'email' => 'danieleceravolo@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Giorgio',
            'surname' => 'Lumia',
            'email' => 'giorgiolumia@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Giuseppe',
            'surname' => 'Indomenico',
            'email' => 'giuseppeindomenico@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Giovanni Jacopo',
            'surname' => 'Speranza',
            'email' => 'giovannisperanza@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Alessandro',
            'surname' => 'Pagliuca',
            'email' => 'alessandropagliuca@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Francesco',
            'surname' => 'Froechlich',
            'email' => 'francescofroechlich@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Arianna',
            'surname' => 'Drovandi',
            'email' => 'ariannadrovandi@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Anas',
            'surname' => 'Hamimi',
            'email' => 'anashamimi@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Filippo',
            'surname' => 'Giampapa',
            'email' => 'filippogiampapa@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Raffaele',
            'surname' => 'Capaldo',
            'email' => 'raffaelecapaldo@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Nicoló',
            'surname' => 'Zibra',
            'email' => 'nicolozibra@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Filippo',
            'surname' => 'Verrone',
            'email' => 'filippoverrone@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Christian',
            'surname' => 'Totaro',
            'email' => 'christiantotaro@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Nicola',
            'surname' => 'Di Quinzio',
            'email' => 'nicoladiquinzio@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Emanuele',
            'surname' => 'Rosella Musico',
            'email' => 'emanuelerosellamusico@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Davide',
            'surname' => 'Caputo',
            'email' => 'davidecaputo@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Christian',
            'surname' => 'Paliotta Moretti',
            'email' => 'christianpaliottamoretti@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Matteo',
            'surname' => 'Aguiari',
            'email' => 'matteoaguiari@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Alessandro',
            'surname' => 'Garganese',
            'email' => 'alessandrogarganese@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Gerson',
            'surname' => 'Membrillo',
            'email' => 'gersonmembrillo@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Jorge Luis',
            'surname' => 'Tenasoa',
            'email' => 'jorgeluistenasoa@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Davide',
            'surname' => 'Sberna',
            'email' => 'davidesberna@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Gianmarco',
            'surname' => 'Incocciati',
            'email' => 'gianmarcoincocciati@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Taddeo',
            'surname' => 'Neri',
            'email' => 'taddeoneri@mail.com',
            'password' => 'bugmaker'
        ],
        [
            'name' => 'Alessio',
            'surname' => 'Logozzo',
            'email' => 'alessiologozzo@mail.com',
            'password' => 'bugmaker'
        ]
    ],

    'types' => [
        [
            'name' => 'Americano',
            'image' => '/img/types/americano.png',
            'rest_image' => '/img/thumbnail/americano.jpg'
        ],
        [
            'name' => 'Cinese',
            'image' => '/img/types/cinese.png',
            'rest_image' => '/img/thumbnail/cinese.jpg'
        ],
        [
            'name' => 'Dolci',
            'image' => '/img/types/dolci.png',
            'rest_image' => '/img/thumbnail/dolci.jpg'
        ],
        [
            'name' => 'Fritti',
            'image' => '/img/types/fritti.png',
            'rest_image' => '/img/thumbnail/fritti.jpg'
        ],
        [
            'name' => 'Gelato',
            'image' => '/img/types/gelato.png',
            'rest_image' => '/img/thumbnail/gelato.jpg'
        ],
        [
            'name' => 'Giapponese',
            'image' => '/img/types/giapponese.png',
            'rest_image' => '/img/thumbnail/giapponese.jpg'
        ],
        [
            'name' => 'Hamburger',
            'image' => '/img/types/hamburger.png',
            'rest_image' => '/img/thumbnail/hamburger.jpg'
        ],
        [
            'name' => 'Italiano',
            'image' => '/img/types/italiano.png',
            'rest_image' => '/img/thumbnail/italiano.jpg'
        ],
        [
            'name' => 'Kebab',
            'image' => '/img/types/kebab.png',
            'rest_image' => '/img/thumbnail/kebab.jpg'
        ],
        [
            'name' => 'Messicano',
            'image' => '/img/types/messicano.png',
            'rest_image' => '/img/thumbnail/messicano.jpg'
        ],
        [
            'name' => 'Panini',
            'image' => '/img/types/panini.png',
            'rest_image' => '/img/thumbnail/panini.jpg'
        ],
        [
            'name' => 'Pinsa',
            'image' => '/img/types/pinsa.png',
            'rest_image' => '/img/thumbnail/pinsa.jpg'
        ],
        [
            'name' => 'Pizza',
            'image' => '/img/types/pizza.png',
            'rest_image' => '/img/thumbnail/pizza.jpg'
        ],
        [
            'name' => 'Poke',
            'image' => '/img/types/poke.png',
            'rest_image' => '/img/thumbnail/poke.jpg'
        ],
        [
            'name' => 'Pollo',
            'image' => '/img/types/pollo.png',
            'rest_image' => '/img/thumbnail/pollo.jpg'
        ],
    ],

    'restaurants' => [
        [
            'name' => 'John\'s Burger',
            'email' => 'johnsburger@mail.com',
            'p_iva' => '17861849275',
            'phone_num' => '3527019384',
            'user_id' => 1,
            'address' => 'Via Roma 17',
            'types' => [
                1,
                7,
            ],
        ],
        [
            'name' => 'Happy Sushi Wok',
            'email' => 'happysushiwok@mail.com',
            'p_iva' => '43928291018',
            'phone_num' => '3896245703',
            'user_id' => 2,
            'address' => 'Via Garibaldi 23',
            'types' => [
                2,
                14,
            ],
        ],
        [
            'name' => 'La Pizzaccia 2',
            'email' => 'lapizzaccia2@mail.com',
            'p_iva' => '72365381927',
            'phone_num' => '3401856972',
            'user_id' => 3,
            'address' => 'Corso Vittorio Emanuele 45',
            'types' => [
                8,
                13,
            ],
        ],
        [
            'name' => 'Los Pollos Hermanos',
            'email' => 'lospolloshermanos@mail.com',
            'p_iva' => '56201947285',
            'phone_num' => '3762418590',
            'user_id' => 4,
            'address' => 'Via Dante 12',
            'types' => [
                10,
                15,
            ],
        ],
        [
            'name' => 'Cioccolato Puro',
            'email' => 'cioccolatopuro@mail.com',
            'p_iva' => '91483057162',
            'phone_num' => '3610925347',
            'user_id' => 5,
            'address' => 'Via Verdi 9',
            'types' => [
                8,
                3,
            ],
        ],
        [
            'name' => 'Halal Kebab & Pizza',
            'email' => 'halalkebabpizza@mail.com',
            'p_iva' => '67482613908',
            'phone_num' => '3941586027',
            'user_id' => 6,
            'address' => 'Corso Umberto I 34',
            'types' => [
                9,
                13,
            ],
        ],
        [
            'name' => 'Carmelo Patatina',
            'email' => 'carmelopatatina@mail.com',
            'p_iva' => '30948761592',
            'phone_num' => '3857314609',
            'user_id' => 7,
            'address' => 'Via XX Settembre 56',
            'types' => [
                4,
                11,
            ],
        ],
        [
            'name' => 'Bar Arturo',
            'email' => 'bararturo@mail.com',
            'p_iva' => '82057391643',
            'phone_num' => '3120867459',
            'user_id' => 8,
            'address' => 'Via Montello 78',
            'types' => [
                8,
                5,
            ],
        ],
        [
            'name' => 'Pinseria Pasta Madre',
            'email' => 'pinseriapastamadre@mail.com',
            'p_iva' => '38576291840',
            'phone_num' => '3495671028',
            'user_id' => 9,
            'address' => 'Via San Francesco 3',
            'types' => [
                8,
                13,
            ],
        ],
        [
            'name' => 'Yoshi',
            'email' => 'yoshi@mail.com',
            'p_iva' => '10295738462',
            'phone_num' => '3618957420',
            'user_id' => 10,
            'address' => 'Via Gramsci 87',
            'types' => [
                6,
                14,
            ],
        ],
        [
            'name' => 'Daje Pizza',
            'email' => 'dajepizza@mail.com',
            'p_iva' => '74920163582',
            'phone_num' => '3932408576',
            'user_id' => 11,
            'address' => 'Via Marconi 25',
            'types' => [
                13,
                4,
            ],
        ],
        [
            'name' => 'La Pizza De Latina',
            'email' => 'lapizzadelatina@mail.com',
            'p_iva' => '51692748301',
            'phone_num' => '3740289615',
            'user_id' => 12,
            'address' => 'Via Leopardi 71',
            'types' => [
                13,
                8,
            ],
        ],
        [
            'name' => 'The King Burger',
            'email' => 'thekingburger@mail.com',
            'p_iva' => '20834971562',
            'phone_num' => '3569481027',
            'user_id' => 13,
            'address' => 'Via Solferino 32',
            'types' => [
                1,
                7,
            ],
        ],
        [
            'name' => 'Mister Ice',
            'email' => 'misterice@mail.com',
            'p_iva' => '93658204127',
            'phone_num' => '3824056917',
            'user_id' => 14,
            'address' => 'Corso Italia 54',
            'types' => [
                5,
                3,
            ],
        ],
        [
            'name' => 'Dama Sushi',
            'email' => 'damasushi@mail.com',
            'p_iva' => '67283915042',
            'phone_num' => '3157690428',
            'user_id' => 15,
            'address' => 'Via Manzoni 8',
            'types' => [
                6,
                2,
            ],
        ],
        [
            'name' => 'La Piazzetta',
            'email' => 'lapiazzetta@mail.com',
            'p_iva' => '52406381972',
            'phone_num' => '3479620513',
            'user_id' => 16,
            'address' => 'Via Veneto 66',
            'types' => [
                8,
                12,
            ],
        ],
        [
            'name' => 'Ciak Burger',
            'email' => 'ciakburger@mail.com',
            'p_iva' => '87369412058',
            'phone_num' => '3908764251',
            'user_id' => 17,
            'address' => 'Via dei Mille 43',
            'types' => [
                7,
                11,
            ],
        ],
        [
            'name' => 'Gli Sfiziosi',
            'email' => 'glisfiziosi@mail.com',
            'p_iva' => '30571692843',
            'phone_num' => '3736501849',
            'user_id' => 18,
            'address' => 'Piazza del Popolo 10',
            'types' => [
                13,
                12,
            ],
        ],
        [
            'name' => 'Pokè Time',
            'email' => 'poketime@mail.com',
            'p_iva' => '61849275301',
            'phone_num' => '3205768914',
            'user_id' => 19,
            'address' => 'Piazza Navona 21',
            'types' => [
                14,
                6,
            ],
        ],
        [
            'name' => 'So...Fritto!',
            'email' => 'sofritto@mail.com',
            'p_iva' => '28914563729',
            'phone_num' => '3647819502',
            'user_id' => 20,
            'address' => 'Piazza di Spagna 37',
            'types' => [
                15,
                4,
            ],
        ],
        [
            'name' => 'Pinsa Romana e Kebab H24',
            'email' => 'pinsakebab@mail.com',
            'p_iva' => '75192048367',
            'phone_num' => '3830947612',
            'user_id' => 21,
            'address' => 'Piazza San Pietro 2',
            'types' => [
                9,
                12,
            ],
        ],
        [
            'name' => 'Dolce Tex Mex',
            'email' => 'docletexmex@mail.com',
            'p_iva' => '42690183572',
            'phone_num' => '3502716849',
            'user_id' => 22,
            'address' => 'Largo Garibaldi 15',
            'types' => [
                10,
                4,
            ],
        ],
        [
            'name' => 'Nanà Ristorante Pizzeria',
            'email' => 'nanaristorantepizzeria@mail.com',
            'p_iva' => '68305721934',
            'phone_num' => '3948106527',
            'user_id' => 23,
            'address' => 'Viale delle Terme 4',
            'types' => [
                8,
                13,
            ],
        ],
        [
            'name' => 'Tonino\'s',
            'email' => 'toninos@mail.com',
            'p_iva' => '24917638570',
            'phone_num' => '3317894506',
            'user_id' => 24,
            'address' => 'Viale dei Giardini 7',
            'types' => [
                1,
                13,
            ],
        ],
        [
            'name' => 'Fujiyama',
            'email' => 'fujiyama@mail.com',
            'p_iva' => '17593648290',
            'phone_num' => '3751026948',
            'user_id' => 25,
            'address' => 'Viale Kennedy 19',
            'types' => [
                2,
                14,
            ],
        ],
        [
            'name' => 'O\' Zuppular',
            'email' => 'ozuppular@mail.com',
            'p_iva' => '63849215726',
            'phone_num' => '3180592764',
            'user_id' => 26,
            'address' => 'Viale Europa 27',
            'types' => [
                3,
                8,
            ],
        ],
        [
            'name' => 'Non solo pizza',
            'email' => 'nonsolopizza@mail.com',
            'p_iva' => '92147536028',
            'phone_num' => '3925437801',
            'user_id' => 27,
            'address' => 'Viale delle Rimembranze 36',
            'types' => [
                4,
                13,
            ],
        ],
        [
            'name' => 'Il Mago del Gelo',
            'email' => 'ilmagodelgelo@mail.com',
            'p_iva' => '36481950792',
            'phone_num' => '3604189572',
            'user_id' => 28,
            'address' => 'Viale XXV Aprile 50',
            'types' => [
                5,
                3,
            ],
        ],
        [
            'name' => 'Moki Sushi',
            'email' => 'mokisushi@mail.com',
            'p_iva' => '58972403615',
            'phone_num' => '3861570942',
            'user_id' => 29,
            'address' => 'Viale Aldo Moro 64',
            'types' => [
                6,
                14,
            ],
        ],
        [
            'name' => 'Ke-Burger',
            'email' => 'keburger@mail.com',
            'p_iva' => '20891734625',
            'phone_num' => '3210948657',
            'user_id' => 30,
            'address' => 'Viale dei Pini 11',
            'types' => [
                7,
                4,
            ],
        ],
        [
            'name' => 'Casa Criscuolo',
            'email' => 'casacriscuolo@mail.com',
            'p_iva' => '78362915047',
            'phone_num' => '3541972086',
            'user_id' => 31,
            'address' => 'Viale dei Cipressi 28',
            'types' => [
                8,
                11,
            ],
        ],
        [
            'name' => 'Ke-Bab',
            'email' => 'kebab@mail.com',
            'p_iva' => '41692837509',
            'phone_num' => '3708629415',
            'user_id' => 32,
            'address' => 'Viale della Libertà 76',
            'types' => [
                9,
                10,
            ],
        ],
        [
            'name' => 'Veracruz',
            'email' => 'veracruz@mail.com',
            'p_iva' => '27059681423',
            'phone_num' => '3458601927',
            'user_id' => 33,
            'address' => 'Viale Gramsci 88',
            'types' => [
                10,
                4,
            ],
        ],
        [
            'name' => 'Happy Food',
            'email' => 'happyfood@mail.com',
            'p_iva' => '49658102734',
            'phone_num' => '3806547912',
            'user_id' => 34,
            'address' => 'Viale Giuseppe Mazzini 14',
            'types' => [
                11,
                4,
            ],
        ],
        [
            'name' => 'La Pinseria',
            'email' => 'lapinseria@mail.com',
            'p_iva' => '81952037649',
            'phone_num' => '3639781042',
            'user_id' => 35,
            'address' => 'Viale Matteotti 33',
            'types' => [
                12,
                8,
            ],
        ],
        [
            'name' => 'Oro di Napoli',
            'email' => 'orodinapoli@mail.com',
            'p_iva' => '35097482163',
            'phone_num' => '3985421760',
            'user_id' => 36,
            'address' => 'Viale Giacomo Leopardi 52',
            'types' => [
                13,
                4,
            ],
        ],
        [
            'name' => 'Pokeria Tiburon',
            'email' => 'pokeriatiburon@mail.com',
            'p_iva' => '69501348721',
            'phone_num' => '3301928576',
            'user_id' => 37,
            'address' => 'Viale Gabriele D\'Annunzio 68',
            'types' => [
                14,
                6,
            ],
        ],
        [
            'name' => 'Il Mago dello Spiedo',
            'email' => 'ilmagodellospiedo@mail.com',
            'p_iva' => '28341679502',
            'phone_num' => '3764029185',
            'user_id' => 38,
            'address' => 'Viale Giuseppe Garibaldi 81',
            'types' => [
                15,
                4,
            ],
        ],
        [
            'name' => 'Make Burger Great Again',
            'email' => 'makeburger@mail.com',
            'p_iva' => '95830621745',
            'phone_num' => '3184907526',
            'user_id' => 39,
            'address' => 'Viale Enrico Fermi 96',
            'types' => [
                1,
                7,
            ],
        ],
        [
            'name' => 'King Kong',
            'email' => 'kingkong@mail.com',
            'p_iva' => '20745986312',
            'phone_num' => '3542097681',
            'user_id' => 40,
            'address' => 'Viale Leonardo da Vinci 5',
            'types' => [
                2,
                6,
            ],
        ],
        [
            'name' => 'Dolci Carie',
            'email' => 'dolcicarie@mail.com',
            'p_iva' => '46815932740',
            'phone_num' => '3831460972',
            'user_id' => 41,
            'address' => 'Viale Alessandro Volta 22',
            'types' => [
                3,
                8,
            ],
        ],
        [
            'name' => 'Troppo Unto',
            'email' => 'troppounto@mail.com',
            'p_iva' => '92740381569',
            'phone_num' => '3472860915',
            'user_id' => 42,
            'address' => 'Viale Galileo Galilei 31',
            'types' => [
                4,
                12,
            ],
        ],
        [
            'name' => 'Diabete Freddo',
            'email' => 'diabetefreddo@mail.com',
            'p_iva' => '61379485206',
            'phone_num' => '3609541827',
            'user_id' => 43,
            'address' => 'Viale Niccolò Machiavelli 47',
            'types' => [
                5,
                3,
            ],
        ],
        [
            'name' => 'Manga Food',
            'email' => 'mangafood@mail.com',
            'p_iva' => '80256391748',
            'phone_num' => '3953748210',
            'user_id' => 44,
            'address' => 'Viale Copernico 63',
            'types' => [
                6,
                11,
            ],
        ],
        [
            'name' => 'Krusty Burger',
            'email' => 'krustyburger@mail.com',
            'p_iva' => '37495128603',
            'phone_num' => '3175649802',
            'user_id' => 45,
            'address' => 'Viale Guglielmo Marconi 72',
            'types' => [
                7,
                4,
            ],
        ],
        [
            'name' => 'Spaghetti Mafia',
            'email' => 'spaghettimafia@mail.com',
            'p_iva' => '65924837150',
            'phone_num' => '3497260183',
            'user_id' => 46,
            'address' => 'Viale Archimede 83',
            'types' => [
                8,
                13,
            ],
        ],
        [
            'name' => 'Notti d\'oriente',
            'email' => 'nottidoriente@mail.com',
            'p_iva' => '52104968730',
            'phone_num' => '3821659047',
            'user_id' => 47,
            'address' => 'Viale dei Caduti 92',
            'types' => [
                9,
                11,
            ],
        ],
        [
            'name' => 'Speedy Gonzales',
            'email' => 'speedygonzales@mail.com',
            'p_iva' => '89635072413',
            'phone_num' => '3130846579',
            'user_id' => 48,
            'address' => 'Viale dei Pini 13',
            'types' => [
                10,
                4,
            ],
        ],
        [
            'name' => 'Pane e Tramezzi',
            'email' => 'panetramezzi@mail.com',
            'p_iva' => '43175628907',
            'phone_num' => '3579420186',
            'user_id' => 49,
            'address' => 'Viale degli Ulivi 29',
            'types' => [
                11,
                8,
            ],
        ],
        [
            'name' => 'Pinsa da Maggika',
            'email' => 'pinsamaggika@mail.com',
            'p_iva' => '78610593427',
            'phone_num' => '3841096257',
            'user_id' => 50,
            'address' => 'Viale dei Fiori 38',
            'types' => [
                12,
                15,
            ],
        ],
        [
            'name' => 'Er Vesuvio',
            'email' => 'ervesuvio@mail.com',
            'p_iva' => '57623019847',
            'phone_num' => '3602419785',
            'user_id' => 51,
            'address' => 'Viale dei Ciliegi 57',
            'types' => [
                13,
                12,
            ],
        ],
        [
            'name' => 'Pokemon Poke',
            'email' => 'pokemonpoke@mail.com',
            'p_iva' => '93218576034',
            'phone_num' => '3940718256',
            'user_id' => 52,
            'address' => 'Viale delle Rose 74',
            'types' => [
                14,
                6,
            ],
        ],
        [
            'name' => 'Ti Spiedo in Due',
            'email' => 'tispiedoindue@mail.com',
            'p_iva' => '24703691582',
            'phone_num' => '3268945071',
            'user_id' => 53,
            'address' => 'Viale degli Oleandri 99',
            'types' => [
                15,
                1,
                2 => 11,
            ],
        ],
        [
            'name' => 'Sekiro',
            'email' => 'sekiro@mail.com',
            'p_iva' => '45678945645',
            'phone_num' => '3451898110',
            'user_id' => 40,
            'address' => 'Via Roma 489',
            'types' => [
                6,
                14,
            ],
        ],
    ],
];
